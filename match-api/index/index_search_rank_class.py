        ...
        for pair in pairs:
            for term in query_tokens:
                if term in self.index:
                    if pair[1] == 'Name':
                        if pair[0] == 'Artist':
                            self.artist_title_scores, self.album_title_scores, self.track_title_scores = self.rank_artist_titles(term, pair[0]) # include flag for artist's album titles scores and artist's track titles scores
                        if pair[0] == 'Album':
                            self.album_title_scores, self.rack_title_scores = self.rank_album_titles(term, pair[0]) # include flag for album's artist titles scores
                        if pair[0] == 'Track':
                            self.track_title_scores = self.rank_track_titles(term, pair[0]) # include flag for track's album titles scores and track's artist titles scores
                        
                    if pair[1] == 'Genres':
                        if pair[0] == 'Track':
                            self.track_genres_scores = self.rank_genres(term, pair[0],  self.doclengths_track_data,  self.track_genres_scores)
                        if pair[0] == 'Album':
                            self.album_genres_scores = self.rank_genres(term, pair[0],  self.doclengths_album_data, self.album_genres_scores)
                        if pair[0] == 'Artist':
                            self.artist_genres_scores = self.rank_genres(term, pair[0],  self.doclengths_artist_data,  self.artist_genres_scores)
                    if pair[1] == 'Tags':
                        if pair[0] == 'Track':
                            self.track_tags_scores = self.rank_tags(term, pair[0], self.doclengths_track_data,  self.track_tags_scores)
                        if pair[0] == 'Album':
                            self.album_tags_scores = self.rank_tags(term, pair[0], self.doclengths_album_data,  self.album_tags_scores)
                        if pair[0] == 'Artist':
                            self.artist_tags_scores = self.rank_tags(term, pair[0], self.doclengths_artist_data,  self.artist_tags_scores)
                else:
                    continue        
        #track dictionary
